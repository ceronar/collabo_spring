...
public class CartController {
    ...
    public ResponseEntity<String> addToCart(@RequestBody CartProductDto dto){
        ...
        if(cart.getCartProducts() != null) { // 장바구니에 아무것도 없을 때 null이라 오류발생
    ...
    @GetMapping("/list/{memberId}") // 특정 사용자의 '카트 상품' 목록을 조회
    public ResponseEntity<List<CartProductResponseDto>> getCartProducts(@PathVariable Long memberId) {
        ...
        if (optionalMember.isEmpty()) { // memberId인 회원 정보가 없을 경우
    ...
    public ResponseEntity<String> updateCartProduct(@PathVariable Long cartProductId, @RequestParam(required = false) Integer quantity) {
        ...
        System.out.println("변경 할 상품 수량 : " + quantity);
        ...
        Optional<CartProduct> cartProductOptional = cartProductService.findCartProductById(cartProductId);
        ...
        cartProductService.saveCartProduct(cartProduct); // Service -> DB 저장

        message = "카트 상품 아이디 " + cartProductId + "번이 '" + quantity + "개'로 수정 되었습니다.";
